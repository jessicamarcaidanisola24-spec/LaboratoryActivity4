...
let dayAsc = true;
function sortByDay(){
  records.sort((a,b)=> dayAsc ? a.day.localeCompare(b.day) : b.day.localeCompare(a.day));
  dayAsc = !dayAsc;
  renderList(records);
}
...
let timeAsc = true;
function sortByTime(){
  records.sort((a,b)=> timeAsc ? a.time.localeCompare(b.time) : b.time.localeCompare(a.time));
  timeAsc = !timeAsc;
  renderList(records);
}
...
document.getElementById('addBtn').onclick = ()=>{
  ...
  if(!s || !d || !t){ alert('Fill required fields'); return; }
  addRecord(s,d,t,r);
  document.getElementById('subject').value=''; document.getElementById('day').value='';
  document.getElementById('time').value=''; document.getElementById('room').value='';
};
document.getElementById('resetBtn').onclick = ()=>{ if(confirm('Clear all?')){ records=[]; ScheduleManager.list=[]; renderList(records);} };
document.getElementById('search').oninput = (e)=> renderList(searchRecords(e.target.value));
document.getElementById('sortDay').onclick = sortByDay;
document.getElementById('sortTime').onclick = sortByTime;
document.getElementById('filterMon').onclick = ()=> renderList(records.filter(r => r.day.toLowerCase().includes('mon')));
